...
double SensorModel::scoreScanBeamModel(const MovingLaserScan& movingScan, const OccupancyGrid& map){
    ...
    float rangeSim;
    float rangeDiff;
    float sumLogLikelihoods = 0.0;
    int numLogLikelihoods = 0;
    int numInvalids = 0;
    for (int i = 0; i < movingScan.size(); i += ray_stride_) {
    	if (movingScan[i].range > maxRange) {
    		numInvalids += 1;
    		continue;
    	}
    	...
    	rangeDiff = movingScan[i].range - rangeSim;
    	sumLogLikelihoods += lnConstCoeff - 0.5 * pow(rangeDiff / stdDev, 2);
    	numLogLikelihoods += 1;
    }
    sumLogLikelihoods += numInvalids * sumLogLikelihoods / numLogLikelihoods;
    double likelihood = exp(sumLogLikelihoods);
    return likelihood;
}
